...
<template>
  <section class="page-head"><div><p class="eyebrow">Área do usuário</p><h1>Minhas multas</h1><p>Multas vinculadas às suas permanências.</p></div></section>
  ...
  <section class="section-block"><div class="section-title"><div><h2>Histórico</h2><p>GET /api/me/fines</p></div></div>
    <StatePanel :loading="fines.isLoading.value" :error="fines.error.value ? getApiError(fines.error.value) : null" :empty="!fines.data.value?.length">
      <div class="table-wrap"><table><thead><tr><th>ID</th><th>Motivo</th><th>Valor</th><th>Permanência</th><th>Veículo</th><th>Início</th><th>Status</th></tr></thead><tbody><tr v-for="fine in fines.data.value" :key="fine.id"><td>#{{ fine.id }}</td><td>{{ reasonLabel(fine.reason) }}</td><td>{{ money(fine.amount) }}</td><td>#{{ fine.stay_id }}</td><td>{{ fine.vehicle?.plate ?? '—' }}</td><td>{{ dateTime(fine.started_at) }}</td><td><StatusBadge :status="fine.status" /></td></tr></tbody></table></div>
    </StatePanel>
  </section>
</template>

...
const loading = computed(() => zones.isLoading.value || vehicles.isLoading.value || stays.isLoading.value || charges.isLoading.value)
const error = computed(() => [zones.error.value, vehicles.error.value, stays.error.value, charges.error.value].find(Boolean))
</script>
...
<template>
  ...
  <StatePanel :loading="loading" :error="error ? getApiError(error) : null">
    ...
    <aside class="notice"><AlertTriangle /><div><strong>Recursos respeitam a API atual</strong><p>Cadastro público, depósito e consulta direta de multas não aparecem porque não possuem endpoints ativos.</p></div></aside>
  </StatePanel>
</template>
